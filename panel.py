...
class PanelImpresiones:
    ...
    def _construir_ui(self):
        ...
        frame_tabla.pack(fill="both", expand=True, padx=12, pady=(0, 12))
        ...
        frame_botones.pack(pady=(0, 12))

        tk.Button(
            frame_botones, text="Imprimir seleccionado",
    ...
    def _actualizar_tabla(self):
        ...
        self.lbl_contador.config(
            text=f"{procesando} pedido(s) listos para imprimir"
        )
    ...
    def _imprimir_seleccionado(self):
        id_p = self._obtener_id_seleccionado()
        if not id_p:
            return

        # Verificar que el pedido esté en PROCESANDO
        pedidos = obtener_pedidos()
        ...
        if estado_actual != "PROCESANDO":
            messagebox.showwarning(
                "Aviso",
                f"Solo se pueden imprimir pedidos en estado PROCESANDO.\n"
        ...
        confirmar = messagebox.askyesno(
            ...
            f"Archivo: {pedido[2]}\n"
            f"Hojas: {pedido[3]} — Monto: ${pedido[4]:.0f}"
        )
        if not confirmar:
            return
        ...
        def _hilo_imprimir():
            ...
            if exito:
                ...
                self.lbl_estado.config(text=f"Pedido #{id_p} impreso correctamente", fg="#2E7D32")
            else:
                self.lbl_estado.config(text=f"Error al imprimir pedido #{id_p}", fg="#C62828")
            self._actualizar_tabla()
